/*
【新店福利】【闪购盲盒】【疯狂砸金蛋】【健康服务】
['1EFRQxA','1EFRRxA','1EFRQwA','1EFRTwg','1EFRTyg']
...
const inviteCodes = [`P04z54XCjVUnoaW5nJcXCCyoR8C6i9QR16e`];
!(async () => {
  await requireConfig();
  if (!cookiesArr[0]) {
...
async function jdHealth() {
  //await helpFriends();
  await jdxdfl_getTaskDetail();
  //await doTask();
  await jdxdfl_getTaskDetail(0);
  await jdsgmh_getTaskDetail();
  //await doTask();
  await jdsgmh_getTaskDetail(0);
  await jdfkzjd_getTaskDetail();
  //await doTask();
  await jdfkzjd_getTaskDetail(0);
  await jdjkfw_getTaskDetail();
  //await doTask();
  await jdjkfw_getTaskDetail(0);
  await jddwg_getTaskDetail();
  //await doTask();
  await jddwg_getTaskDetail(0);
...
function jdxdfl_getTaskDetail(get=1) {
  return new Promise(resolve => {
          ...
          if (safeGet(data)) {
            ...
            if (data.data.bizCode === 0) {
              ...
              if(get)
                $.taskVos.map(item => {
                  if (item.taskType === 14) {
                    console.log(`\n您的1EFRQxA${$.name}好友助力邀请码：${item.assistTaskDetailVo.taskToken}\n`)
                    message += `\n您的1EFRQxA${$.name}好友助力邀请码：${item.assistTaskDetailVo.taskToken}\n`
                  }
                })
            }
...
function jdsgmh_getTaskDetail(get=1) {
  return new Promise(resolve => {
    $.post(taskPostUrl("healthyDay_getHomeData", {"appId":"1EFRRxA","taskToken":""}, ), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
          ...
          if (safeGet(data)) {
            ...
            if (data.data.bizCode === 0) {
              ...
              if(get)
                $.taskVos.map(item => {
                  if (item.taskType === 14) {
                    console.log(`\n您的1EFRRxA${$.name}好友助力邀请码：${item.assistTaskDetailVo.taskToken}\n`)
                    message += `\n您的1EFRRxA${$.name}好友助力邀请码：${item.assistTaskDetailVo.taskToken}\n`
                  }
                })
            }
...
function jdfkzjd_getTaskDetail(get=1) {
  return new Promise(resolve => {
    $.post(taskPostUrl("healthyDay_getHomeData", {"appId":"1EFRQwA","taskToken":""}, ), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
          ...
          if (safeGet(data)) {
            ...
            if (data.data.bizCode === 0) {
              ...
              if(get)
                $.taskVos.map(item => {
                  if (item.taskType === 14) {
                    console.log(`\n您的1EFRQwA${$.name}好友助力邀请码：${item.assistTaskDetailVo.taskToken}\n`)
                    message += `\n您的1EFRQwA${$.name}好友助力邀请码：${item.assistTaskDetailVo.taskToken}\n`
                  }
                })
            }
...
function jdjkfw_getTaskDetail(get=1) {
  return new Promise(resolve => {
          ...
          if (safeGet(data)) {
            ...
            if (data.data.bizCode === 0) {
              ...
              if(get)
                $.taskVos.map(item => {
                  if (item.taskType === 14) {
                    console.log(`\n您的1EFRTwg${$.name}好友助力邀请码：${item.assistTaskDetailVo.taskToken}\n`)
                    message += `\n您的1EFRTwg${$.name}好友助力邀请码：${item.assistTaskDetailVo.taskToken}\n`
                  }
                })
            }
...
function jddwg_getTaskDetail(get=1) {
  return new Promise(resolve => {
    $.post(taskPostUrl("healthyDay_getHomeData", {"appId":"1EFRTyg","taskToken":""}, ), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
          ...
          if (safeGet(data)) {
            ...
            if (data.data.bizCode === 0) {
              ...
              if(get)
                $.taskVos.map(item => {
                  if (item.taskType === 14) {
                    console.log(`\n您的1EFRTyg${$.name}好友助力邀请码：${item.assistTaskDetailVo.taskToken}\n`)
                    message += `\n您的1EFRTyg${$.name}好友助力邀请码：${item.assistTaskDetailVo.taskToken}\n`
                  }
                })
            }
